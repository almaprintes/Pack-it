(()=>{
const VERSION='DEV 0.0.61';
document.querySelectorAll('.version,.splash-version').forEach(e=>e.textContent=VERSION);
const music=new Audio('assets/audio/music-cozy-jazz-v1.mp3?v=0061');
...
function bell(ctx,when,freq,gain=.055,dur=.8){const out=ctx.createGain();out.gain.setValueAtTime(.0001,when);out.gain.exponentialRampToValueAtTime(gain,when+.012);out.gain.exponentialRampToValueAtTime(.0001,when+dur);out.connect(ctx.destination);[[1,1],[2.01,.24],[3.98,.08]].forEach(([mul,level])=>{const o=ctx.createOscillator(),g=ctx.createGain();o.type='sine';o.frequency.setValueAtTime(freq*mul,when);g.gain.value=level;o.connect(g);g.connect(out);o.start(when);o.stop(when+dur+.03)})}
...
function boxSlide(){const ctx=getCtx();if(!ctx)return;const t=ctx.currentTime+.01,d=.48,n=ctx.createBuffer(1,Math.floor(ctx.sampleRate*d),ctx.sampleRate),a=n.getChannelData(0);for(let i=0;i<a.length;i++){const x=i/a.length;a[i]=(Math.random()*2-1)*(1-x)*(.65+.35*Math.sin(i*.19))}const src=ctx.createBufferSource(),filter=ctx.createBiquadFilter(),g=ctx.createGain();src.buffer=n;filter.type='bandpass';filter.frequency.setValueAtTime(420,t);filter.frequency.exponentialRampToValueAtTime(150,t+d);filter.Q.value=.7;g.gain.setValueAtTime(.0001,t);g.gain.exponentialRampToValueAtTime(.035,t+.025);g.gain.exponentialRampToValueAtTime(.0001,t+d);src.connect(filter);filter.connect(g);g.connect(ctx.destination);src.start(t);src.stop(t+d);const tap=ctx.createOscillator(),tg=ctx.createGain();tap.type='triangle';tap.frequency.setValueAtTime(115,t);tap.frequency.exponentialRampToValueAtTime(72,t+.07);tg.gain.setValueAtTime(.035,t);tg.gain.exponentialRampToValueAtTime(.0001,t+.09);tap.connect(tg);tg.connect(ctx.destination);tap.start(t);tap.stop(t+.1)}
function startAudio(){if(started)return;started=true;getCtx();music.play().catch(()=>{})}
function nextBoxSound(){clearTimeout(restoreTimer);fadeMusic(.18,180);boxSlide()}
