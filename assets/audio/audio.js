(()=>{
const VERSION='DEV 0.0.52';
document.querySelectorAll('.version,.splash-version').forEach(e=>e.textContent=VERSION);
const music=new Audio('assets/audio/music-cozy-jazz-v1.mp3?v=0052');
const pack=new Audio('assets/audio/pack-it-short.mp3?v=0052');
...
function stopPack(){clearTimeout(restoreTimer);pack.pause();pack.currentTime=0;fadeMusic(.18,180)}
document.querySelector('#startGame')?.addEventListener('click',startAudio,{passive:true});
document.querySelector('#next')?.addEventListener('pointerdown',stopPack,{passive:true});
