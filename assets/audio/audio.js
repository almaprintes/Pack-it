...
function boxSlide(){const ctx=getCtx();if(!ctx)return;const t=ctx.currentTime+.005,d=.58,n=ctx.createBuffer(1,Math.floor(ctx.sampleRate*d),ctx.sampleRate),a=n.getChannelData(0);for(let i=0;i<a.length;i++){const x=i/a.length;a[i]=(Math.random()*2-1)*(1-x)*(.7+.3*Math.sin(i*.17))}const src=ctx.createBufferSource(),filter=ctx.createBiquadFilter(),g=ctx.createGain();src.buffer=n;filter.type='bandpass';filter.frequency.setValueAtTime(650,t);filter.frequency.exponentialRampToValueAtTime(190,t+d);filter.Q.value=.65;g.gain.setValueAtTime(.0001,t);g.gain.exponentialRampToValueAtTime(.22,t+.018);g.gain.exponentialRampToValueAtTime(.0001,t+d);src.connect(filter);filter.connect(g);g.connect(ctx.destination);src.start(t);src.stop(t+d);const tap=ctx.createOscillator(),tg=ctx.createGain();tap.type='triangle';tap.frequency.setValueAtTime(175,t);tap.frequency.exponentialRampToValueAtTime(68,t+.14);tg.gain.setValueAtTime(.32,t);tg.gain.exponentialRampToValueAtTime(.0001,t+.17);tap.connect(tg);tg.connect(ctx.destination);tap.start(t);tap.stop(t+.18)}
function startAudio(){if(started)return;started=true;getCtx();music.play().catch(()=>{})}
function nextBoxSound(){clearTimeout(restoreTimer);stopChime();clearInterval(fadeMusic._t);music.volume=.012;boxSlide();restoreTimer=setTimeout(()=>fadeMusic(.18,500),650)}
